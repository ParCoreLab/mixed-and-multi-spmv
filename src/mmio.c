...
int mm_read_banner(FILE *f, MM_typecode *matcode) {
  ...
  if (strcmp(mtx, MM_MTX_STR) == 0) mm_set_matrix(matcode);
  // added by Erhan Tezcan
  else if (strcmp(mtx, MM_VEC_STR) == 0)
...
int mm_read_mtx_crd_entry(FILE *f, int *I, int *J, double *real, double *img, MM_typecode matcode) {
  if (mm_is_complex(matcode)) {
    if (fscanf(f, "%d %d %lg %lg", I, J, real, img) != 4) return MM_PREMATURE_EOF;
  } else if (mm_is_real(matcode)) {
